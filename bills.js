...
function billLogoSafe(a){
  try{
    return typeof logo==='function'?logo(a):'';
  }catch(e){
    console.warn('Bill logo failed',e,a);
  }
  let label=String(a?.institution||a?.name||'CC').trim().slice(0,3).toUpperCase()||'CC';
...
function setPayAmount(mode){
  if(!settling)return;
  document.querySelectorAll('.payMode').forEach(b=>b.classList.remove('active'));
  let btn=document.querySelector(`[data-paymode="${mode}"]`);
  if(btn)btn.classList.add('active');
  if(mode==='full')payAmount.value=Number(settling.remaining||0);
  if(mode==='half')payAmount.value=Math.ceil(Number(settling.remaining||0)/2);
  if(mode==='custom'){
    payAmount.focus();
    payAmount.select();
  }
...
function settlePaymentHistory(bill){
  let payments=billPayments(bill);
  if(!payments.length)return '<div class="sub">No payments recorded yet.</div>';
  return payments.map(p=>`<div class="historyRow"><span>${txnDate(p)} - ${accountLabel(p.from)}</span><b>${peso(p.amount)}</b></div>`).join('');
}

function openSettle(id){
  settling=data.bills.find(b=>b.id===id);
  if(!settling){
    alert('This bill is no longer available to settle.');
    return;
  }
...
function recurringIsPaidThisMonth(r){
  let key=recurringMonthKey(new Date());
  return (data.txns||[]).some(t=>t.recurringId===r.id&&(t.occurrenceMonth===key||String(t.occurrenceKey||'').slice(0,7)===key));
}

function recurringTxnFor(r,occ){
  return {
    id:uid(),
    type:r.type,
    amount:Number(r.amount||0),
    fee:0,
    category:r.category||r.name||r.type,
    date:new Date().toISOString(),
    from:r.accountId,
    to:null,
    recurringId:r.id,
    occurrenceKey:recurringOccurrenceKey(r,occ),
    occurrenceMonth:recurringMonthKey(occ),
    note:'Recurring: '+(r.name||r.type)
  };
}

function payRecurring(id){
  let r=data.recurring.find(x=>x.id===id);
  if(!r)return;
  let a=data.accounts.find(x=>x.id===r.accountId);
  if(!a)return alert('Choose an account for this recurring item first.');
  if(r.enabled===false)return alert('Enable this recurring item first.');
  if(recurringIsPaidThisMonth(r))return toastMsg((r.type==='Income'?'Received':'Paid')+' for this month already');
  let occ=currentRecurringOccurrence(r);
  let t=recurringTxnFor(r,occ);
  if(!applyTxn(t))return alert('Could not record this recurring transaction.');
  data.txns.push(t);
  persist();
  toastMsg(r.type==='Income'?'Marked received':'Marked paid');
}

function recurringSortValue(r){
  let disabled=r.enabled===false;
  let paid=recurringIsPaidThisMonth(r);
  let next=nextRecurringDate(r);
  return {status:disabled?2:paid?1:0,next:next.getTime()||0,name:String(r.name||r.category||'')};
}

function sortRecurringItems(items){
  return (items||[]).slice().sort((a,b)=>{
    let aa=recurringSortValue(a);
    let bb=recurringSortValue(b);
    return (aa.status-bb.status)||(aa.next-bb.next)||aa.name.localeCompare(bb.name);
  });
}

function recurringMonthTotal(items){
  return (items||[]).filter(r=>r&&typeof r==='object'&&r.enabled!==false).reduce((sum,r)=>sum+Math.abs(Number(r.amount||0)),0);
}

function renderRecurringSummary(items){
  let pill=document.getElementById('recurringTotalPill');
  if(!pill)return;
  let active=(items||[]).filter(r=>r&&typeof r==='object'&&r.enabled!==false);
  if(!active.length){pill.classList.add('hide');pill.textContent='';return}
  pill.textContent='Monthly '+peso(recurringMonthTotal(active));
  pill.classList.remove('hide');
}

function recurringCardHtml(r){
  let a=(data.accounts||[]).find(x=>x&&x.id===r.accountId);
  let next=nextRecurringDate(r);
  let paid=recurringIsPaidThisMonth(r);
  let action=r.type==='Income'?'Received':'Pay';
  let doneLabel=r.type==='Income'?'Received':'Paid';
  let disabled=r.enabled===false;
  let acct=a?htmlText(a.name)+' ('+htmlText(a.institution||a.type)+')':'Missing account';
  let schedule=paid?doneLabel:'Every '+htmlText(r.day)+recurringDaySuffix(r.day);
  let payButton=paid?'':`<button class="tiny primary" ${disabled?'disabled':''} onclick="payRecurring('${jsString(r.id)}')">${action}</button>`;
  return `<div class="recurringCard ${paid?'isPaid':''}"><div class="recTop"><div class="recIdentity"><b>${htmlText(r.name)}</b><div class="recMeta">${htmlText(r.type)} - ${htmlText(r.category||'Other')} - ${acct}</div></div><span class="recPill ${paid?'paid':'due'}">${schedule}</span></div><div class="minirow" style="margin-top:10px"><span>Amount</span><b>${peso(r.amount)}</b></div><div class="minirow"><span>Next</span><b>${next.toLocaleDateString('en-PH',{month:'short',day:'numeric',year:'numeric'})}</b></div><div class="recActions"><button class="tiny" onclick="openRecurring('${jsString(r.id)}')">Edit</button><button class="tiny ${disabled?'':'danger'}" onclick="toggleRecurring('${jsString(r.id)}')">${disabled?'Enable':'Disable'}</button>${payButton}</div></div>`;
}

function renderRecurring(){
  let el=document.getElementById('recurringList');
  if(!el)return;
  let items=(data.recurring||[]).filter(r=>r&&typeof r==='object');
  let arr=sortRecurringItems(items);
  renderRecurringSummary(items);
  el.innerHTML=arr.length
    ? arr.map(recurringCardHtml).join('')
    : '<div class="row"><span class="sub">No recurring items yet. Add salary, subscriptions, MP2, utilities, or monthly bills.</span></div>';
}
