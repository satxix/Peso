...
function accountCurrentMonthTxns(accountId,months){months=months||3;let now=new Date(),start=new Date(now.getFullYear(),now.getMonth()-(months-1),1),end=new Date(now.getFullYear(),now.getMonth()+1,1);return accountTxns(accountId).filter(t=>{let d=new Date(t.date||Date.now());return d>=start&&d<end})}
...
function accountDetailHeroHtml(a,main,label){let baseNote=a.type==='Credit Card'?'':'<div class="sub accountBaseNote">Starting '+peso(a.ledgerBaseBalance||0)+'</div>';let creditBar=a.type==='Credit Card'?'<div class="bar"><i style="width:'+Math.min(100,((a.outstanding||0)/(a.limit||1))*100)+'%"></i></div>':'';return '<section class="detailHero"><div class="detailTop">'+logo(a)+'<div><div class="name">'+htmlText(a.name,'Account')+'</div><div class="inst">'+htmlText(a.institution||a.type)+' - '+htmlText(a.type)+'</div></div></div><div class="small" style="margin-top:14px">'+label+'</div><div class="detailAmount">'+peso(main)+'</div>'+baseNote+creditBar+'</section>'}
function accountDetailStatsHtml(a,st,openBill,nextSt,nextDue){if(a.type==='Credit Card')return '<div class="statBox"><span class="small">Credit Limit</span><b>'+peso(a.limit||0)+'</b></div><div class="statBox"><span class="small">Available</span><b>'+peso((a.limit||0)-(a.outstanding||0))+'</b></div><div class="statBox"><span class="small">Next Statement</span><b>'+displayDate(nextSt)+'</b></div><div class="statBox"><span class="small">Next Due</span><b>'+(openBill?displayDate(openBill.dueDate):displayDate(nextDue))+'</b></div>';return '<div class="statBox"><span class="small">Income</span><b>'+peso(st.income)+'</b></div><div class="statBox"><span class="small">Expense</span><b>'+peso(st.expense)+'</b></div><div class="statBox"><span class="small">Transfers In</span><b>'+peso(st.transferIn)+'</b></div><div class="statBox"><span class="small">Transfers Out</span><b>'+peso(st.transferOut+st.fees)+'</b></div>'}
function statementBilledAmount(b){
  let original=Number(b&&b.amount||0);
  if(original>0)return original;
  let remaining=Math.max(0,Number(b&&b.remaining||0));
  let paid=(data.txns||[]).filter(t=>t&&t.type==='Card Payment'&&t.billId===b.id).reduce((sum,t)=>sum+Number(t.amount||0),0);
  return remaining+paid;
}
function accountStatementHistoryHtml(a,statements,openBill){if(a.type!=='Credit Card')return '';let rows=statements.length?statements.map(b=>'<div class="statementMini compactStatement"><div><b class="statementPeriod">'+compactBillPeriod(b)+'</b><div class="sub">Due '+displayDate(b.dueDate)+'</div></div><div class="statementAmt"><b>'+peso(statementBilledAmount(b))+'</b><span class="statusPill '+statusClass(billStatus(b))+'">'+billStatus(b)+'</span></div></div>').join(''):'<div class="emptyState">No statement history yet. Credit card purchases will create statement records automatically.</div>';return '<div class="section"><h2>Statement History</h2>'+(openBill?'<button class="ghost" onclick="openSettle(\''+jsString(openBill.id)+'\')">Settle Current</button>':'')+'</div><div class="statementList">'+rows+'</div>'}
function accountRecentActivityHtml(tx){return '<div class="section"><h2>Recent Activity</h2><span class="sub">Last 3 months</span></div><div class="detailList">'+(tx.length?tx.map(t=>txnRow(t,true)).join(''):'<div class="emptyState">No activity in the last 3 months for this account.</div>')+'</div>'}
function openAccountDetail(id){let a=data.accounts.find(x=>x.id===id);if(!a)return;let st=accountStats(id),tx=accountCurrentMonthTxns(id),main=a.type==='Credit Card'?Number(a.outstanding||0):Number(a.balance||0),label=a.type==='Credit Card'?'Outstanding':'Current Balance';let allStatements=a.type==='Credit Card'?cardAllBills(a.id).filter(b=>statementBilledAmount(b)>0):[],statements=allStatements.slice(0,6),openBill=allStatements.find(b=>billStatus(b)!=='Paid');let nextSt=a.type==='Credit Card'?nextStatementDate(a):null,nextDue=a.type==='Credit Card'?dueFromStatement(a,nextSt):null;accountDetailBody.innerHTML=accountDetailHeroHtml(a,main,label)+'<div class="statGrid">'+accountDetailStatsHtml(a,st,openBill,nextSt,nextDue)+'</div><div class="sheetActions"><button class="primary" onclick="editAccountFromDetail(\''+jsString(a.id)+'\')">Edit Account</button><button onclick="openTxnFromDetail()">Add Transaction</button></div><button class="dangerBtn" onclick="deleteAccountById(\''+jsString(a.id)+'\')">Delete Account</button>'+accountStatementHistoryHtml(a,statements,openBill)+accountRecentActivityHtml(tx);showModal();accountDetailSheet.classList.add('show')}
