...
function accountCurrentMonthTxns(accountId){let now=new Date(),start=new Date(now.getFullYear(),now.getMonth(),1),end=new Date(now.getFullYear(),now.getMonth()+1,1);return accountTxns(accountId).filter(t=>{let d=new Date(t.date||Date.now());return d>=start&&d<end})}
...
function accountRecentActivityHtml(tx){return '<div class="section"><h2>Recent Activity</h2></div><div class="detailList">'+(tx.length?tx.map(t=>txnRow(t,true)).join(''):'<div class="emptyState">No activity this month for this account.</div>')+'</div>'}
