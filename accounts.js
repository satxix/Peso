...
function accountCurrentMonthTxns(accountId){let now=new Date(),start=new Date(now.getFullYear(),now.getMonth(),1),end=new Date(now.getFullYear(),now.getMonth()+1,1);return accountTxns(accountId).filter(t=>{let d=new Date(t.date||Date.now());return d>=start&&d<end}).slice(0,12)}
